...
# Load votes from file (only for existing candidates)
def load_votes():
    if os.path.exists(VOTES_FILE):
        with open(VOTES_FILE, "r") as f:
            for line in f:
                name, count = line.strip().split(",")
                if name in candidates:   # ✅ Only update if candidate exists
                    candidates[name] = int(count)

# Save votes to file (only current candidates)
def save_votes():
...
def show_results():
    ...
    # Only current candidates are used (already ensured)
